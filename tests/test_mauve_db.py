...
class TestMauveDB:
    ...
    def _auto_mongomock_patch(self, mongomock_client, monkeypatch):
        ...
        monkeypatch.setattr(
            mauve_db, "client_factory", value=lambda: mongomock_client,
        )
        yield
        monkeypatch.undo()
    ...
    @pytest.mark.parametrize(
        "filter_kwarg, many", [(SCHEME_KWARG, True), (SCHEME_KWARG, False),]
    )
    ...
    @pytest.mark.parametrize(
        "filter_kwarg, many", [({"foo": "bar"}, True), ({"foo": "bar"}, False),]
    )
    ...
    @pytest.mark.parametrize(
        "filter_kwarg, expected", [({}, 3), (SCHEME_KWARG, 3), ({"name": "Sienna"}, 1)]
    )
